...
SERVICE_DEFS: dict[str, dict[str, str]] = {
    "dawnclaw": {
        "pid_file": "dawnclaw.pid",
        "heartbeat": "dawnclaw.json",
        "cwd_marker": "dawnclaw",
    },
    "dawn-feishu": {
        "pid_file": "dawn-feishu.pid",
        "heartbeat": "dawn-feishu.json",
        "cwd_marker": "dawn-feishu",
    },
    "dawn-discord": {
        "pid_file": "dawn-discord.pid",
        "heartbeat": "dawn-discord.json",
        "cwd_marker": "dawn-discord",
    },
}
...
def _write_json_atomic(path: Path, data: Any) -> None:
    ...
    tmp = path.with_name(
        f"{path.name}.tmp-{int(time.time() * 1000)}-{uuid.uuid4().hex[:6]}"
    )
...
def _active_runtime_manifest_path() -> Optional[Path]:
    pointer = _read_active_runtime_pointer()
    manifest_path = str(
        pointer.get("manifest_path") or pointer.get("manifestPath") or ""
    ).strip()
    if manifest_path:
...
def _poll_im_command_result(
    im_type: str, command_id: str, timeout_ms: int = 10000
) -> dict[str, Any]:
    ...
    return {
        ...
        "error": "Timed out waiting for IM service response",
    }
...
def _poll_background_job(
    job_id: str, timeout_ms: int = 2000
...
def _poll_command_response(
    im_type: str, request_id: str, timeout_ms: int = 15000
) -> dict[str, Any]:
...
def _find_folder_by_display_name(
    name: str, im_type: Optional[str] = None
) -> Optional[str]:
    for avatar_id in _list_avatar_ids():
        cfg = _read_json(_avatar_config_path(avatar_id), {})
        for folder, meta in (cfg.get("groupConfigs") or {}).items():
            if (meta or {}).get("displayName") == name and (
                not im_type or (meta or {}).get("imType") == im_type
            ):
                return folder
    return None
...
def _find_avatar_feishu_source_group(avatar_id: str) -> str:
    ...
    group_configs = (
        cfg.get("groupConfigs") if isinstance(cfg.get("groupConfigs"), dict) else {}
    )
...
def _resolve_context_secret_path(
    group_folder: str, im_type: str = ""
) -> Optional[Path]:
    ...
    if not resolved_im_type:
        resolved_im_type = (
            _infer_im_type_from_folder_name(group_folder) or _default_im_type()
        )
    if not resolved_im_type:
        return None
...
def _verify_context_token(token: str) -> dict[str, Any]:
    ...
    if not secret_path:
        raise DawnImError(
            "Could not resolve the IM type for the current Dawn context token"
        )
    try:
    ...
    if not tokens_dir:
        raise DawnImError(
            "Could not resolve the IM token directory for the current Dawn context token"
        )
    ...
    if (
        _binding_value(token_meta, "token_id", "tokenId") != token_id
        or _binding_value(token_meta, "signature") != signature_hex
    ):
...
def _resolve_current_context(thread_id: str = "") -> dict[str, Any]:
    resolved_thread_id = str(
        thread_id or os.environ.get("CODEX_THREAD_ID") or ""
    ).strip()
    ...
    if not binding_path:
        raise DawnImError(
            f"Missing current Dawn context binding for thread {resolved_thread_id}"
        )
    ...
    return {
        ...
        "imType": str(
            token_payload.get("imType")
            or _binding_value(binding, "imType", "im_type")
            or ""
        ).strip(),
        "contextToken": token,
...
def _resolve_with_current(
    flag_name: str, explicit: str, current: str, force_current: bool
) -> str:
    ...
    if force_current:
        ...
        if explicit_value and explicit_value != current_value:
            raise DawnImError(
                f"--{flag_name} conflicts with current context ({current_value})"
            )
...
def _prepare_feishu_create_params(
    ...
    if not members:
        ...
        if source_group_resolved not in channels:
            raise DawnImError(
                f"source_group not found in feishu registered channels: {source_group_resolved}"
            )
...
def _add_group_binding(
    ...
    if inherit_group and "codex" not in next_entry:
        ...
        source_codex = (
            source_entry.get("codex") if isinstance(source_entry, dict) else None
        )
...
def _remove_group_bindings(
    folder: str, avatar_id: Optional[str] = None
) -> dict[str, Any]:
    ...
    for aid in candidates:
        ...
        cfg = _read_json(cfg_path, {})
        current_groups = (
            cfg.get("groups") if isinstance(cfg.get("groups"), list) else []
        )
...
def _list_channels_once(im_type: str, timeout_ms: int = 8000) -> dict[str, Any]:
    ...
    if not result.get("success"):
        raise DawnImError(
            f"list_channels failed: {result.get('error', 'unknown error')}"
        )
...
def _require_im_type(im_type: str) -> str:
    if im_type not in IM_TYPES:
        raise DawnImError(
            f"Invalid im_type '{im_type}'. Use one of: {', '.join(IM_TYPES)}"
        )
    return im_type
...
def _normalize_background_item(item: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    group_folder = str(
        _background_item_field(item, "groupFolder", "group_folder", "group") or ""
    ).strip()
...
def _summarize_background_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "counts": counts,
        "terminal": all(
            str((item or {}).get("status") or "").strip()
            in {
                "done",
                "failed_terminal",
                "delivery_failed",
...
def cmd_service_restart(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for item in results:
        ...
        if status in {"restarted", "running"}:
            ...
            status in {"omitted", "blocked"}
            and _runtime_component_presence(SERVICE_COMPONENTS.get(service_name, ""))
            != "present"
        ):
            continue
        else:
    ...
    if failed_services:
        ...
        message = (
            f"Services restarted successfully: {', '.join(sorted(restarted_services))}"
        )
    else:
...
def cmd_im_register_channel(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not folder:
        base = re.sub(
            r"[^a-z0-9]+", "-", (args.display_name or args.channel_id).lower()
        ).strip("-")
        folder = f"{args.im_type}-{base or 'channel'}"

    _add_group_binding(
        args.avatar_id, folder, args.im_type, args.display_name or folder
    )
    ...
    if folder not in groups:
        raise DawnImError(
            "verification_failed: folder missing from avatar config groups"
        )

    channels = _list_channels_once(args.im_type, timeout_ms=args.timeout_ms)
    if folder not in channels:
        raise DawnImError(
            "verification_failed: folder missing from list_channels after register"
        )
...
def cmd_im_create_channel(args: argparse.Namespace) -> dict[str, Any]:
    ...
    current_ctx = (
        _resolve_current_context(getattr(args, "thread_id", ""))
        if force_current
        else _try_resolve_current_context(getattr(args, "thread_id", ""))
    )
    ...
    if not im_type:
        raise DawnImError(
            "im_type is required, or use --current inside a Dawn IM conversation"
        )
    ...
    if folder not in channels:
        raise DawnImError(
            "verification_failed: created folder missing from list_channels"
        )
    ...
    if folder not in groups or gc.get("imType") != im_type:
        raise DawnImError(
            "verification_failed: config binding mismatch after create_channel"
        )
...
def cmd_im_delete_channel(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.leave_service and delete_result.get("success") is False:
        raise DawnImError(
            f"delete_channel failed on service side: {delete_result.get('error', 'unknown error')}"
        )

    if (
        delete_result.get("success") is False
        and delete_result.get("error") == "Timed out waiting for IM service response"
    ):
        raise DawnImError(
            "IM service unreachable. Channel may still be active. Retry when service is online."
        )
    ...
    if cfg_check_avatar:
        ...
        if folder in groups:
            raise DawnImError(
                "verification_failed: folder still present in avatar groups after delete"
            )

    channels = _list_channels_once(args.im_type, timeout_ms=args.timeout_ms)
    if folder in channels:
        raise DawnImError(
            "verification_failed: folder still present in list_channels after delete"
        )
            ...
            "apps": [
                {
                    ...
                    "defaultAvatarId": str(
                        config.get("defaultAvatarId") or config.get("avatarId") or ""
                    ).strip(),
...
def _feishu_app_add(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not name or not app_id or (not app_secret and not secret_ref):
        raise DawnImError(
            "app_add requires name, appId, and either appSecret or secretRef"
        )
...
def cmd_background_dispatch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if isinstance(raw, list):
        ...
        raise DawnImError(
            "items-json must decode to a JSON array or an object with items"
        )
    ...
    job_id = (
        request_job_id or f"bg_job_{int(time.time() * 1000)}_{uuid.uuid4().hex[:6]}"
    )
    items = [
        _normalize_background_item(item, index) for index, item in enumerate(items_raw)
    ]
    ...
    if job:
        ...
        response["message"] = (
            "Background job submitted; waiting for Dawn Settings to accept it."
        )
    return response
...
def cmd_codex_new_chat(args: argparse.Namespace) -> dict[str, Any]:
    ...
    current_ctx = (
        _resolve_current_context(getattr(args, "thread_id", ""))
        if (force_current or not group)
        else None
    )

    group = _resolve_with_current(
        "group", group, (current_ctx or {}).get("groupFolder", ""), force_current
    )
    if not group:
        raise DawnImError(
            "group is required, or use --current inside a Dawn IM conversation"
        )

    same_group_as_current = bool(
        current_ctx and group == str(current_ctx.get("groupFolder") or "").strip()
    )
    im_type = _resolve_with_current(
        "im-type",
        str(args.im_type or "").strip(),
...
def cmd_codex_fork_chat(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source_group = _resolve_with_current(
        "source-group",
        source_group,
        (current_ctx or {}).get("groupFolder", ""),
        force_current,
    )
    if not source_group:
        raise DawnImError(
            "source_group is required, or use --current inside a Dawn IM conversation"
        )

    same_group_as_current = bool(
        current_ctx
        and source_group == str(current_ctx.get("groupFolder") or "").strip()
    )
    ...
    if not avatar_id:
        raise DawnImError(
            "avatar_id is required when source_group is not bound to any avatar"
        )
    ...
    if im_type == "feishu":
        create_params, source_group_used, source_group_auto_selected = (
            _prepare_feishu_create_params(
                avatar_id,
                [],
                source_group,
                args.timeout_ms,
    ...
    bind_result = _add_group_binding(
        avatar_id, folder, im_type, args.name, inherit_group=source_group
    )
    ...
    if folder not in channels:
        raise DawnImError(
            "verification_failed: forked channel missing from list_channels"
        )
...
def _resolve_avatar_id(args: argparse.Namespace) -> str:
    ...
    if not avatar_id:
        raise DawnImError(
            "No --avatar-id provided and no active avatar set in global.json"
        )
    return avatar_id
...
def _normalize_codex_dict(raw: Any, group_override: bool = False) -> dict[str, Any]:
    ...
    allowed_fields = (
        GROUP_CODEX_FIELDS
        if group_override
        else tuple(CODEX_CONFIG_FIELD_TO_ARG.keys())
    )
...
def _merge_codex_effective(
    default_codex: dict[str, Any], group_codex: dict[str, Any]
) -> dict[str, Any]:
...
def _load_avatar_default_codex(avatar_id: str) -> dict[str, Any]:
    return _normalize_codex_dict(
        _read_json(_avatar_config_path(avatar_id), {}).get("codex", {})
    )
...
def _request_codex_capabilities_live() -> dict[str, Any]:
    ...
    def read_response(request_id: str, method: str, timeout_sec: float) -> Any:
        ...
        if detail:
            raise DawnImError(
                f"Timed out waiting for codex response: {method}. stderr: {detail}"
            )
    ...
    raw_providers = (
        providers_result.get("data") if isinstance(providers_result, dict) else []
    )
    ...
    env_map = {
        ...
        and str(item.get("envKey", "") or "").strip()
    }
    if env_map:
...
def _load_codex_capabilities() -> dict[str, Any]:
    ...
    cached_models = (
        cached.get("models") if isinstance(cached.get("models"), list) else []
    )
    cached_providers = (
        cached.get("providers") if isinstance(cached.get("providers"), list) else []
    )
    ...
    if AVATARS_DIR.exists():
        for avatar_dir in AVATARS_DIR.iterdir():
            if not avatar_dir.is_dir():
                continue
            _collect_model_provider_pairs(
                _read_json(avatar_dir / "config.json", {}), pairs
            )
            _collect_model_provider_pairs(
                _read_json(avatar_dir / "state.json", {}), pairs
            )
...
def _normalize_working_directory_value(
    value: str, *, allow_empty: bool
) -> Optional[str]:
    trimmed = str(value or "").strip()
    if not trimmed:
    ...
    if not expanded.is_absolute():
        raise DawnImError(
            f"workingDirectory must be an absolute path after expansion: {trimmed}"
        )
...
def _normalize_prompt_path_value(avatar_id: str, field_name: str, value: str) -> str:
    ...
    resolved = (
        raw_path if raw_path.is_absolute() else AVATARS_DIR / avatar_id / raw_path
    )
    if not resolved.exists():
        raise DawnImError(f"{field_name} file does not exist: {resolved}")
    if resolved.is_dir():
        raise DawnImError(
            f"{field_name} must point to a file, not a directory: {resolved}"
        )
...
def _resolve_model_entry(
    models: list[dict[str, Any]], submitted: str, provider_hint: str = ""
) -> tuple[str, str]:
    trimmed = str(submitted or "").strip()
    ...
    if known_models:
        raise DawnImError(
            f"Unknown model '{trimmed}'. Known models: {', '.join(known_models)}"
        )
    raise DawnImError(f"Unknown model '{trimmed}'")


def _validate_and_normalize_model_selection(
    ...
    models = (
        ...
        if isinstance(capabilities.get("models"), list)
        else []
    )
    providers = (
        capabilities.get("providers")
        if isinstance(capabilities.get("providers"), list)
        else []
    )
    ...
    canonical_model, inferred_provider = _resolve_model_entry(
        models, trimmed_model, canonical_provider
    )
...
def _build_codex_views(avatar_id: str, group: str) -> dict[str, dict[str, Any]]:
    ...
    inherited_default_codex = (
        _load_avatar_default_codex("_default") if avatar_id != "_default" else {}
    )
    default_codex = _normalize_codex_dict(config.get("codex", {}))
    group_configs = (
        config.get("groupConfigs")
        if isinstance(config.get("groupConfigs"), dict)
        else {}
    )
    ...
    avatar_effective_codex = _merge_codex_effective(
        inherited_default_codex, default_codex
    )
    effective_codex = (
        _merge_codex_effective(avatar_effective_codex, group_codex)
        if group
        else avatar_effective_codex
    )
    return {
...
def _assess_effective_codex_validity(
    avatar_id: str, effective_codex: dict[str, Any]
) -> tuple[str, list[str]]:
    warnings: list[str] = []
    unknown = False
    try:
        _normalize_approval_policy_value(
            str(effective_codex.get("approvalPolicy", "never")), allow_empty=False
        )
        _normalize_sandbox_policy_value(
            str(effective_codex.get("sandboxPolicy", "workspace-write")),
            allow_empty=False,
        )
        _normalize_working_directory_value(
            str(effective_codex.get("workingDirectory", "")), allow_empty=False
        )
        ...
        if base_path:
            ...
            if raw.is_absolute():
                ...
                raise DawnImError(
                    f"baseInstructions cannot escape the avatar directory: {base_path}"
                )
        ...
        if dev_path:
            ...
            if raw.is_absolute():
                ...
                raise DawnImError(
                    f"developerInstructions cannot escape the avatar directory: {dev_path}"
                )
...
def _find_avatars_for_group(group_folder: str) -> list[str]:
    ...
    for avatar_id in _list_avatar_ids():
        ...
        if (
            group_folder in {str(g).strip() for g in groups if str(g).strip()}
            or group_folder in group_configs
        ):
...
def _resolve_codex_target(
    args: argparse.Namespace, require_group: bool = False
) -> tuple[str, str]:
    force_current = bool(getattr(args, "current", False))
    ...
    if require_group and not group:
        raise DawnImError(
            "group is required for group scope, or use --current inside a Dawn IM conversation"
        )
...
def cmd_codex_config_read(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scope = str(getattr(args, "scope", "") or "").strip() or (
        "effective" if group else "default"
    )
    if scope not in CODEX_READ_SCOPES:
        raise DawnImError(
            f"Invalid scope '{scope}'. Must be one of: {', '.join(CODEX_READ_SCOPES)}"
        )
    ...
    config_validity, validation_warnings = _assess_effective_codex_validity(
        avatar_id, effective_codex
    )
    return {
        "ok": True,
        "avatarId": avatar_id,
...
def cmd_codex_config_set(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scope = requested_scope or (
        ...
        if bool(
            ...
            or str(getattr(args, "group", "") or "").strip()
        )
        else "default"
    )
    if scope not in CODEX_SET_SCOPES:
        raise DawnImError(
            f"Invalid scope '{scope}'. Must be one of: {', '.join(CODEX_SET_SCOPES)}"
        )
    ...
    unset_fields = [
        str(field or "").strip()
        for field in getattr(args, "unset", []) or []
    ...
    if scope == "default":
        ...
        if reset_group_override:
            raise DawnImError(
                "--reset-group-override is only supported with --scope group"
            )

        candidate_codex = dict(default_codex)
        if "approvalPolicy" in submitted_codex:
            candidate_codex["approvalPolicy"] = _normalize_approval_policy_value(
                submitted_codex["approvalPolicy"], allow_empty=False
            )
        if "sandboxPolicy" in submitted_codex:
            candidate_codex["sandboxPolicy"] = _normalize_sandbox_policy_value(
                submitted_codex["sandboxPolicy"], allow_empty=False
            )
        if "workingDirectory" in submitted_codex:
            candidate_codex["workingDirectory"] = _normalize_working_directory_value(
                submitted_codex["workingDirectory"], allow_empty=False
            )
        if "baseInstructions" in submitted_codex:
            candidate_codex["baseInstructions"] = _normalize_prompt_path_value(
                avatar_id, "baseInstructions", submitted_codex["baseInstructions"]
            )
        if "developerInstructions" in submitted_codex:
            candidate_codex["developerInstructions"] = _normalize_prompt_path_value(
                avatar_id,
                "developerInstructions",
                submitted_codex["developerInstructions"],
            )
        if "model" in submitted_codex:
            candidate_codex["model"] = str(submitted_codex["model"] or "").strip()
        if "modelProvider" in submitted_codex:
            candidate_codex["modelProvider"] = str(
                submitted_codex["modelProvider"] or ""
            ).strip()
        ...
        if "model" in submitted_codex or "modelProvider" in submitted_codex:
            ...
            if submitted_model and not submitted_provider:
                canonical_model, canonical_provider = (
                    _validate_and_normalize_model_selection(
        ...
        if (
            getattr(args, "base_instructions", None) is not None
            or getattr(args, "developer_instructions", None) is not None
        ):
            raise DawnImError(
                "group scope only supports model, modelProvider, approvalPolicy, sandboxPolicy, workingDirectory"
            )

        invalid_unsets = [
            field for field in unset_fields if field not in GROUP_CODEX_FIELDS
        ]
        if invalid_unsets:
            raise DawnImError(
        ...
        if "approvalPolicy" in submitted_codex:
            normalized = _normalize_approval_policy_value(
                submitted_codex["approvalPolicy"], allow_empty=True
            )
        ...
        if "sandboxPolicy" in submitted_codex:
            normalized = _normalize_sandbox_policy_value(
                submitted_codex["sandboxPolicy"], allow_empty=True
            )
            if normalized is None:
        ...
        if "workingDirectory" in submitted_codex:
            normalized = _normalize_working_directory_value(
                submitted_codex["workingDirectory"], allow_empty=True
            )
            if normalized is None:
        ...
        if (
            ...
            explicit_provider = str(
                candidate_group_codex.get("modelProvider", "") or ""
            ).strip()
            if explicit_model:
                canonical_model, canonical_provider = (
                    _validate_and_normalize_model_selection(
...
def cmd_mcp_call(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if tool == "dawn_im_status":
        return cmd_im_status(
            ns(im_type=raw.get("im_type", None), timeout_ms=timeout_ms)
        )
    if tool == "dawn_im_list_channels":
        return cmd_im_list_channels(
            ns(im_type=raw.get("im_type", None), timeout_ms=timeout_ms)
        )
    if tool == "dawn_im_create_channel":
        return cmd_im_create_channel(
            ns(
                im_type=raw.get("im_type", ""),
                name=raw.get("name", ""),
                avatar_id=raw.get("avatar_id", raw.get("avatarId", "")),
                ...
                prefer_current_avatar=bool(
                    raw.get(
                        "prefer_current_avatar", raw.get("preferCurrentAvatar", True)
                    )
                ),
                allow_private_chat=_coerce_bool(
                    raw.get("allow_private_chat", raw.get("allowPrivateChat", True)),
                    default=True,
                ),
    ...
    if tool == "dawn_im_register_channel":
        return cmd_im_register_channel(
            ns(
                im_type=raw.get("im_type", ""),
    ...
    if tool == "dawn_background_dispatch":
        ...
        return cmd_background_dispatch(
            ns(
    ...
    if tool == "dawn_codex_config_read":
        return cmd_codex_config_read(
            ns(
                avatar_id=raw.get("avatar_id", raw.get("avatarId", "")),
                group=raw.get(
                    "group", raw.get("group_folder", raw.get("groupFolder", ""))
                ),
                current=bool(raw.get("current", False)),
                thread_id=raw.get("thread_id", raw.get("threadId", "")),
                scope=raw.get("scope", ""),
    ...
    if tool == "dawn_codex_config_set":
        ...
        return cmd_codex_config_set(
            ns(
                ...
                base_instructions=raw.get(
                    "base_instructions", raw.get("baseInstructions")
                ),
                developer_instructions=raw.get(
                    "developer_instructions", raw.get("developerInstructions")
                ),
                unset=unset_fields,
                reset_group_override=bool(
                    raw.get(
                        "reset_group_override", raw.get("resetGroupOverride", False)
                    )
                ),
            )
        )
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Dawn IM management via direct IPC (no MCP)"
    )
    sp = p.add_subparsers(dest="command", required=True)

    def add_common(
        cmd: argparse.ArgumentParser, allow_empty_channel: bool = True
    ) -> None:
    ...
    c.add_argument(
        "--current",
        action="store_true",
        help="Use current Dawn IM context defaults when available",
    )
    c.add_argument(
        "--thread-id", default="", help="Explicit thread id (default: CODEX_THREAD_ID)"
    )
    c.add_argument(
        "--prefer-current-avatar",
        action="store_true",
        help="Prefer current context avatar over implicit defaults",
    )
    c.add_argument(
        "--allow-private-chat",
        action="store_true",
    ...
    c.add_argument(
        "--items-json", required=True, help="JSON array of items or object with items[]"
    )
    ...
    c.add_argument(
        "--current",
        action="store_true",
        help="Require and use the current Dawn IM context",
    )
    c.add_argument(
        "--thread-id", default="", help="Explicit thread id (default: CODEX_THREAD_ID)"
    )
    ...
    c.add_argument(
        "--include-settings",
        action="store_true",
        help="Reserved; restarting Dawn Settings itself is not supported here.",
    )
    ...
    c.add_argument(
        "--scope",
        choices=CODEX_READ_SCOPES,
        default=None,
        help="default, group, effective",
    )
    ...
    c.add_argument(
        "--scope", choices=CODEX_SET_SCOPES, default=None, help="default or group"
    )
    c.add_argument("--model", default=None, help="Model name (e.g. claude-opus-4-6)")
    c.add_argument(
        "--model-provider",
        default=None,
        help="Model provider (openai, openrouter, minimax)",
    )
    c.add_argument(
        "--approval-policy",
        default=None,
        help="never, on-request, on-failure, untrusted",
    )
    c.add_argument(
        "--sandbox-policy",
        default=None,
        help="workspace-write, danger-full-access, read-only, external-sandbox",
    )
    c.add_argument("--working-directory", default=None, help="Working directory path")
    c.add_argument(
        "--base-instructions", default=None, help="Base instructions file path"
    )
    c.add_argument(
        "--developer-instructions",
        default=None,
        help="Developer instructions file path",
    )
    c.add_argument(
        "--unset",
        action="append",
        default=[],
        help="Clear a group override field; repeatable",
    )
    c.add_argument(
        "--reset-group-override",
        action="store_true",
        help="Clear all codex overrides for the target group",
    )
