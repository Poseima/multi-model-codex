...
def make_check(
    name: str, status: str, summary: str, details: Optional[dict[str, Any]] = None
) -> dict[str, Any]:
    payload = {
        "name": name,
        "status": status,
...
def read_active_pointer() -> tuple[Optional[dict[str, Any]], dict[str, Any]]:
    ...
    return payload, make_check(
        ...
        "Found active Dawn runtime pointer",
        {
            "path": str(ACTIVE_RUNTIME_PATH),
            "bundleVersion": str(
                payload.get("bundle_version") or payload.get("bundleVersion") or ""
            ).strip(),
            "slotPath": str(
                payload.get("slot_path") or payload.get("slotPath") or ""
            ).strip(),
            "manifestPath": str(
                payload.get("manifest_path") or payload.get("manifestPath") or ""
            ).strip(),
        },
    )
...
def load_manifest(
    ...
    components = (
        payload.get("components") if isinstance(payload.get("components"), dict) else {}
    )
    return payload, make_check(
        "runtime_manifest",
        "ready",
        "Loaded active runtime manifest",
        {
            "path": str(manifest_path),
            "runtimeVersion": str(
                payload.get("runtimeVersion") or payload.get("runtime_version") or ""
            ).strip(),
...
def read_runtime_status() -> tuple[Optional[dict[str, Any]], dict[str, Any]]:
    ...
    components = (
        result.get("components") if isinstance(result.get("components"), dict) else {}
    )
...
def normalize_component_entry(entry: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "version": str(entry.get("version") or "").strip(),
        "statusReason": str(
            entry.get("statusReason") or entry.get("status_reason") or ""
        ).strip(),
    }


def connector_check(
    im_type: str, components: dict[str, Any]
...
def overall_status(checks: list[dict[str, Any]], connector_ready_count: int) -> str:
    if any(check["status"] == "failed" for check in checks):
        return "failed"
    if any(
        check["status"] == "blocked" for check in checks if check["name"] in CORE_CHECKS
    ):
        return "blocked"
...
def main() -> int:
    ...
    if isinstance(runtime_status, dict):
        components = (
            runtime_status.get("components")
            if isinstance(runtime_status.get("components"), dict)
            else {}
        )
