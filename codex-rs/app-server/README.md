...
To continue a stored session, call `thread/resume` with the `thread.id` you previously recorded. The response shape matches `thread/start`, and no additional notifications are emitted. You can also pass the same configuration overrides supported by `thread/start`, including `personality` and `approvalsReviewer`:
...
To branch from a stored session, call `thread/fork` with the `thread.id`. This creates a new thread id and emits a `thread/started` notification for it. You can pass the same override-style inputs as `thread/start`, including `promptProfilePath`, and `ephemeral: true` when the fork should stay in-memory only:
