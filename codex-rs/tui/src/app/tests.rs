...
#[tokio::test]
async fn backtrack_selection_preserves_data_image_urls_for_prompt_edit_branch() {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    ...
    let selection = app
        ...
        .expect("backtrack selection");
    assert_eq!(
        selection.prompt.remote_image_urls,
        vec![data_image_url.clone()]
    );
    ...
    match event {
        AppEvent::ForkSessionForPromptEdit {
            ...
            assert_eq!(prompt.text, "please inspect this");
            assert_eq!(prompt.remote_image_urls, vec![data_image_url]);
        }
        other => panic!("expected prompt edit fork event, got {other:?}"),
    }
}
