...
impl ChatWidget {
    ...
    pub(crate) fn open_provider_popup(&mut self) {
        ...
        let mut providers: Vec<(String, String)> = self
            ...
            .collect();
        providers.sort_by(|left, right| left.1.cmp(&right.1).then(left.0.cmp(&right.0)));
    ...
    fn open_switch_account_popup(&mut self) {
        ...
        let items: Vec<SelectionItem> = candidates
            ...
            .map(|candidate| {
                            ...
                            Ok(()) => {
                                tx.send(AppEvent::InsertHistoryCell(Box::new(
                                    history_cell::new_info_event(
                                        ...
                                        Some(
                                            "Run /new to start a fresh session with the new auth."
                                                .to_string(),
                                        ),
                                    ),
                                )));
                ...
                SelectionItem {
                    name: candidate.name,
                    description: candidate.description.or(candidate.disabled_reason.clone()),
                    is_current: false,
                    is_disabled: candidate.disabled_reason.is_some(),
                    actions,
                    dismiss_on_select: true,
                    ..Default::default()
                }
        ...
        let mut header = ColumnRenderable::new();
        header.push(Line::from("Select Account".bold()));
        header.push(Line::from(
