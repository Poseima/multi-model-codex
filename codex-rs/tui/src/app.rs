...
fn guardian_approvals_mode() -> AutoReviewMode {
    auto_review_mode()
}
...
impl App {
    ...
    async fn surface_pending_inactive_thread_interactive_requests(&mut self) {
    ...
    async fn try_submit_active_thread_op_via_app_server(
        ...
        match op.view() {
            ...
            AppCommandView::Other(Op::Archive) => {
    ...
    async fn refresh_pending_thread_approvals(&mut self) {
        ...
        let active_side_parent_thread_id = self.active_side_parent_thread_id();
        for (thread_id, store) in channels {
            if Some(thread_id) == self.active_thread_id
                || Some(thread_id) == active_side_parent_thread_id
            {
                continue;
            }
    ...
    async fn infer_session_for_thread_notification(
        ...
        session.prompt_profile = notification.thread.prompt_profile.clone();
        session.prompt_profile_path = notification.thread.prompt_profile_path.clone();
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::LoadPromptProfile { path } => {
                return self.handle_load_prompt_profile(tui, app_server, path).await;
            }
            AppEvent::ShowPromptProfile => {
                return self.handle_show_prompt_profile(app_server).await;
            }
            AppEvent::ClearPromptProfile => {
                return self.handle_clear_prompt_profile(tui, app_server).await;
            }
