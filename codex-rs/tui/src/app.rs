...
fn prompt_profile_summary_lines(prompt_profile: &PromptSource) -> Vec<Line<'static>> {
...
impl App {
    ...
    async fn current_prompt_profile(&self) -> Result<Option<PromptSource>> {
        ...
        let thread = self
            ...
            .await
            .wrap_err("Failed to access the active thread")?;
        Ok(thread.prompt_profile().await)
    }
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<AppRunControl> {
        match event {
            ...
            AppEvent::ForkCurrentSession => {
                ...
                if let Some(path) = self.chat_widget.rollout_path() {
                    ...
                    if path.exists() {
                        match self
                            .server
                            .fork_thread(usize::MAX, self.config.clone(), path.clone(), None, false)
                            .await
                        {
                            Ok(forked) => {
                                self.shutdown_current_thread().await;
            ...
            AppEvent::LoadPromptProfile { path } => {
                let resolved_path =
                    match AbsolutePathBuf::resolve_path_against_base(path, &self.config.cwd) {
                        Ok(path) => path.into_path_buf(),
                        Err(err) => {
                            self.chat_widget.add_error_message(format!(
                                "Failed to resolve prompt profile path: {err}"
                            ));
                            return Ok(AppRunControl::Continue);
                        }
                    };
            ...
            AppEvent::ShowPromptProfile => match self.current_prompt_profile().await {
            ...
            AppEvent::ClearPromptProfile => match self.current_prompt_profile().await {
                Ok(Some(prompt_profile)) => {
                    let prompt_profile_name = prompt_profile_display_name(&prompt_profile);
                    let summary = session_summary(
                        self.chat_widget.token_usage(),
                    ...
                    if let Some(path) = self.chat_widget.rollout_path().filter(|path| path.exists())
                    {
                            ...
                            .await
                        {
                            Ok(forked) => {
                                ...
                                self.chat_widget.add_info_message(
                                    format!(
                                        "Cleared prompt profile `{prompt_profile_name}`."
                                    ),
                                    Some(
