...
impl<'a> ChatRequestBuilder<'a> {
    ...
    pub fn build(self, provider: &Provider) -> Result<ChatRequest, ApiError> {
        ...
        for (idx, item) in input.iter().enumerate() {
            match item {
                ...
                ResponseItem::LocalShellCall {
                    ...
                    let reasoning = reasoning_by_anchor_index.get(&idx).map(String::as_str);
                    let tool_call_id = call_id.as_deref().or(id.as_deref()).unwrap_or_default();
        ...
        ensure_tool_results_follow_tool_calls(&mut messages);
...
/// Ensures every `role: "tool"` message immediately follows the `role: "assistant"`
/// message whose `tool_calls` array contains the matching id. Some providers
/// (e.g. MiniMax) reject requests where other messages (assistant text, reasoning)
/// appear between tool_calls and their results.
fn ensure_tool_results_follow_tool_calls(messages: &mut Vec<Value>) {
    let mut i = 0;
    while i < messages.len() {
        ...
        // Walk backwards to find the nearest preceding non-tool message.
        let mut preceding_non_tool = None;
        for j in (0..i).rev() {
            if messages[j].get("role").and_then(Value::as_str) != Some("tool") {
                preceding_non_tool = Some(j);
                break;
            }
        }

        // Check if it's correctly placed: preceding non-tool is an assistant
        // with tool_calls containing our id.
        if let Some(j) = preceding_non_tool {
            let msg = &messages[j];
            if msg.get("role").and_then(Value::as_str) == Some("assistant")
                && msg
                    ...
                    .is_some_and(|calls| {
                        calls
                            .iter()
                            .any(|tc| tc.get("id").and_then(Value::as_str) == Some(&tool_call_id))
                    })
            {
                i += 1;
                continue;
        ...
        // Misplaced: find the assistant message that owns this tool_call_id and
        // move the tool result to be right after it (and any existing tool results).
        ...
        for j in 0..messages.len() {
            ...
            {
                let mut insert_pos = j + 1;
                while insert_pos < messages.len()
                    && messages[insert_pos].get("role").and_then(Value::as_str) == Some("tool")
                {
                    insert_pos += 1;
                }
                messages.insert(insert_pos, removed);
                inserted = true;
                break;
            }
        }
        if !inserted {
            // No matching tool_calls found; drop orphan tool result.
            continue;
        }
        // Don't increment i; the current position now has a different element.
    }
}
...
mod tests {
    ...
    #[test]
    fn tool_result_reordered_when_assistant_text_appears_between_call_and_output() {
        // Simulates: model returned text + tool_calls, session stored
        // [FunctionCall, assistant_text, FunctionCallOutput] in that order.
        let prompt_input = vec![
            ResponseItem::Message {
                id: None,
                role: "user".to_string(),
        ...
        // Tool call must be immediately followed by tool result.
        assert_eq!(messages[2]["role"], "assistant");
        assert!(messages[2]["tool_calls"].is_array());
        assert_eq!(messages[3]["role"], "tool");
        assert_eq!(messages[3]["tool_call_id"], "call-1");

        // Assistant text moved after the tool result.
        assert_eq!(messages[4]["role"], "assistant");
        assert_eq!(messages[4]["content"], "Let me check...");
    }
}
