...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let session_prefix = "--codex-realtime-call-boundary\r\n\
         ...
         \r\n";
    let session_suffix = "\r\n--codex-realtime-call-boundary--\r\n";
    assert!(body.starts_with(session_prefix));
    assert!(body.ends_with(session_suffix));
    let session = &body[session_prefix.len()..body.len() - session_suffix.len()];
    let parsed_session: Value =
        serde_json::from_str(session).context("multipart session body should be json")?;
    assert_eq!(
        parsed_session,
        json!({
            "audio": {
