    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("guardian test runtime")
                .block_on(guardian_parallel_reviews_fork_from_last_committed_trunk_history_inner())
        })?
        .join()
        .map_err(|_| anyhow::anyhow!("guardian test thread panicked"))?
}
